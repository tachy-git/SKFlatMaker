...
# define outputFile path from inputFiles path
inputPath = (options.inputFiles)[0].split('/')
directory = inputPath[5].split('_z')
outputPath = "/gv0/Users/taehee/HerwigSample/SKFlatNtuple/Zmass{}/{}".format(directory[1],directory[0])
try:
    os.makedirs(outputPath)
except OSError as e:
    if e.errno != os.errno.EEXIST:
        raise
fileNum = inputPath[6].replace('.','_').split('_')[1]
outputFilePath = "{}/SKFlatNtuple_{}.root".format(outputPath,fileNum)

process.TFileService = cms.Service("TFileService",
  fileName = cms.string( outputFilePath )
)
